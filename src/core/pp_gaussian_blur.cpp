...
{
	new (&_blur_horizontal) Shader("resources/shaders/gaussian_blur_parametric.comp", string_set{ "HORIZONTAL"s });
	_blur_horizontal.link();
	_blur_horizontal.setPostBarrier(Shader::Barrier::Image);

	new (&_blur_vertical) Shader("resources/shaders/gaussian_blur_parametric.comp");
	_blur_vertical.link();
